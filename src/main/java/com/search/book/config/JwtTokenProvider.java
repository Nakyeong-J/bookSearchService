...
public class JwtTokenProvider {
    ...
    private String expirationTime = String.valueOf(30000 * 60000 * 1000L);
    ...
    public String createToken(Authentication authentication) {
        ...
        return Jwts.builder().setClaims(claims).setSubject(username).setIssuedAt(createdDate).setExpiration(expirationDate).signWith(secretKey).compact();
    }
    ...
    public boolean validateToken(String token) {
        try {
            Jws<Claims> claims = Jwts.parserBuilder().setSigningKey(this.secretKey).build().parseClaimsJws(token); // parseClaimsJws will check expiration date. No need do here.
